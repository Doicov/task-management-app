...
const TaskList: React.FC = observer(() => {
  ...
  return (
    <Box sx={{marginTop: 1}}>
      <List sx={{ width: '100%',  display: 'grid', gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: 3 }}>
      {taskStore.filteredTasks.map((task) => (
          <Card key={task.id} 
            sx={{ 
              padding: "0 10px 0 10px", background: "rgba(20, 20, 50, 0.4)",  border: "1px solid rgba(50, 20, 100, 1)", borderRadius: "12px", boxShadow: "12px 11px 21px 0px rgba(29, 34, 37, 0.48)", transition: "transform 0.4s cubic-bezier(.25,.46,.45,.94), box-shadow 0.4s cubic-bezier(.25,.46,.45,.94)",
              "&:hover": {
            ...
            <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%"}}>
                    <Box sx={{display: "flex", alignItems: "center", justifyContent: "space-between", padding: '20px 10px 5px 10px'}}>
                        <Typography variant="h6" color="#e1e0e0" onClick={() => taskStore.toggleTaskStatus(task.id)} sx={{cursor: "pointer", textDecoration: task.status === "Completed" ? "line-through" : "none", color: task.status === "Completed" ? "#a9a9a9" : "#e1e0e0", }}>
                          {task.title}
                        </Typography>
                    ...
                    <Typography variant="body2" sx={{textDecoration: task.status === "Completed" ? "line-through" : "none", color: task.status === "Completed" ? "#979797" : "#b9b9b9", flexGrow: 1, overflow: "hidden", padding: '10px 10px 5px 10px' }} >{task.description}</Typography>
                    
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 10px 20px 10px' }}>
                      <Box marginTop='12px'>
                        <Typography variant="body2" sx={{ display: "flex", alignItems: "center", gap: "5px", color: "#b9b9b9", fontSize: 12  }}>
                        <EventNote sx={{ fontSize: "14px" }} />
                            Due: {task.dueDate}
                        </Typography>
                        <Typography variant="body2"  sx={{display: "flex", alignItems: "center", gap: "5px", color: "#b9b9b9", fontSize: 12, marginTop: "1px",}}>
                        <AccessTime sx={{ fontSize: "14px" }} />
                            Created: {task.createdAt}
                        </Typography>
                      </Box>
                      <Box>
                        <IconButton sx={{ paddingRight: 1}} onClick={() => setEditTask(task)}>
